...
function getUnsplashImage(width = 800, height = 600, query = "") {
...
function getRandomStock(min = 10, max = 100) {
    return Math.floor(Math.random() * (max - min + 1)) + min;
}
// interface BrandData {
//   title: string;
...
async function main() {
    ...
    for (const department of createdDepartments) {
        const cats = categoryData[department.title];
        for (const catName of cats) {
            const category = await prisma.category.create({
                data: {
                    name: catName,
                    slug: generateSlug(catName),
                    image: getUnsplashImage(400, 300, catName.replace(/\s+/g, "-").toLowerCase()),
                    bannerImage: getUnsplashImage(1200, 400, catName.replace(/\s+/g, "-").toLowerCase()),
    ...
    for (const category of createdCategories) {
        const products = productNames[category.name] || ["Premium Product 1", "Premium Product 2"];
        for (const productName of products) {
            ...
            const product = await prisma.product.create({
                data: {
                    ...
                    imageUrl: getUnsplashImage(600, 600, productName.replace(/\s+/g, "-").toLowerCase()),
                    productImages: [
                        getUnsplashImage(600, 600, productName.replace(/\s+/g, "-").toLowerCase()),
                        getUnsplashImage(600, 600, productName.replace(/\s+/g, "-").toLowerCase()),
                        getUnsplashImage(600, 600, productName.replace(/\s+/g, "-").toLowerCase()),
                    ],
                    ...
                    stockQty: stockQty,
                    lowStockAlert: 10,
                    discount: Math.random() > 0.7 ? Math.floor(Math.random() * 20) + 5 : null, // 5-25% discount
    ...
    for (const banner of banners) {
        await prisma.banner.create({
            data: {
                ...
                mobileImageUrl: getUnsplashImage(800, 600, banner.title.replace(/\s+/g, "-").toLowerCase()),
