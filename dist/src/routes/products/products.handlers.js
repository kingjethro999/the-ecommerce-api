...
export const top = async (c) => {
    ...
    const transformedProducts = products.map((product) => {
        ...
        const uniqueOrders = new Set(product.orderItems.map((item) => item.order.id));
        // Calculate total amount (assuming price * quantity per order item)
        const total = product.orderItems.reduce((sum, item) => sum + item.price * item.quantity, 0);
        return {
            id: product.id,
            name: product.name,
            total,
...
export const dashboardList = async (c) => {
    const products = await prisma.product.findMany({
        select: {
            id: true,
            name: true,
