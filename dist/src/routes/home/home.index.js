...
router.get("/", (c) => {
    ...
    <li class="flex items-center bg-white p-4 rounded-lg shadow-sm border-l-4 border-emerald-500">
      <span class="feature-icon text-2xl mr-3">${escapeHtml(feature.icon)}</span>
  ...
  `)
    ...
    <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200 hover:border-emerald-300 transition-colors">
      <h3 class="font-semibold text-emerald-600 text-lg">${escapeHtml(endpoint.name)}</h3>
  ...
  `)
    ...
    <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
      <a href="${option.link}" class="text-xl font-medium mb-3 text-emerald-600 block hover:text-emerald-700 transition-colors">${escapeHtml(option.title)}</a>
      <ol class="list-decimal pl-5 space-y-1 mb-3">
        ${option.steps
        .map((step) => `<li class="text-sm text-gray-700">${escapeHtml(step)}</li>`)
        .join("")}
      </ol>
      <p class="text-sm text-gray-500 bg-gray-50 p-3 rounded">${escapeHtml(option.note)}</p>
    </div>
  `)
        .join("");
