...
async function getActiveProducts() {
    ...
    const activeProducts = stripeProducts.data.filter((item) => item.active === true);
...
export const create = async (c) => {
    ...
    const amount = checkoutProducts.reduce((sum, item) => sum + item.price * item.quantity, 0);
    // Creating Stripe Non existing Stripe Products
    let activeProducts = await getActiveProducts();
    try {
        for (const product of checkoutProducts) {
            const stripeProduct = activeProducts?.find((stripeProduct) => stripeProduct?.name?.toLowerCase() === product?.name?.toLowerCase());
...
export const stripeWebhook = async (c) => {
...
export const confirmPayment = async (c) => {
    const data = c.req.valid("json");
    await handleConfirmPayment(data.paymentIntentId);
    return c.json({ received: true, error: "" }, HttpStatusCodes.OK);
};
async function handleConfirmPayment(paymentIntentId) {
    try {
...
async function handlePaymentIntentFailed(paymentIntent) {
    try {
        ...
        console.log(`Payment failed for order: ${existingOrder?.orderNumber || "Not found"}`);
    }
    catch (error) {
