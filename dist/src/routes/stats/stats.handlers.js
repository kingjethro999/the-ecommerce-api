...
export const stats = async (c) => {
    try {
        ...
        const [prevCustomers, prevOrders, prevProducts, prevCategories] = await Promise.all([
            prisma.user.count({
                where: {
                    role: "USER",
                    ...previousDateFilter,
...
export const briefItems = async (c) => {
    // Products per category
    const categories = await prisma.category.findMany({
        select: {
            id: true,
    ...
    return c.json({ productOptions, brandOptions, categoryOptions }, HttpStatusCodes.OK);
};
